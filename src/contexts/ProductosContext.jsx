...
export function ProductosProvider({ children }) {
    ...
    function obtenerProductos() {
        return(
            new Promise((res, rej) => {
                fetch('https://6828f64b6075e87073a566e4.mockapi.io/productos')
                    .then((respuesta) =>
                        respuesta.json()
                    )
                    .then((datos) => {
    ...
    const agregarProducto = (producto) => {
        return(
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://6828f64b6075e87073a566e4.mockapi.io/productos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto){
        return(
            new Promise(async(res, rej) => {
            try {
                const respuesta = await fetch(`https://6828f64b6075e87073a566e4.mockapi.io/productos/${producto.id}`, {
                    method: 'PUT',
                    headers: {
                    'Content-Type': 'application/json',
                    },
    ...
    const eliminarProducto = (id) => {
        ...
        if (confirmar) {
            return(
                new Promise(async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://6828f64b6075e87073a566e4.mockapi.io/productos/${id}`, {
                            method: 'DELETE',
                        });
